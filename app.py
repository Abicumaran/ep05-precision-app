...
BASE_COL_ALIASES = {
    ...
    "Device": ["Device", "serialNumber", "serial_number", "deviceId", "device_id"],
}
...
def evaluate_normalization_methods(df: pd.DataFrame, analyte_pair_map: Dict[str, Tuple[str, str]], methods: List[str]) -> pd.DataFrame:
    ...
    summary["normalization_score_lower_is_better"] = (
        0.40 * summary["mid_ref_day_CV_pct"].fillna(summary["all_ref_day_CV_pct"]) +
        0.20 * summary["all_ref_day_CV_pct"] +
        0.15 * summary["all_device_day_CV_pct"] +
        0.15 * summary["iqr_pctbias"] +
        0.10 * sep_penalty
    )
    summary["recommended_for_analyte"] = False
    for label, sub in summary.groupby("analyte"):
        idx = sub["normalization_score_lower_is_better"].replace([np.inf, -np.inf], np.nan).idxmin()
        if pd.notna(idx):
    ...
    ).sort_values(["overall_score_lower_is_better", "recommended_analyte_count"], ascending=[True, False])
    summary = summary.merge(overall, on="normalization_method", how="left")
    return summary.sort_values(["overall_score_lower_is_better", "analyte", "normalization_score_lower_is_better"])
    ...
    with zipfile.ZipFile(zip_buf, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        if not norm_comparison.empty:
            ...
            recommendation = pd.DataFrame([{
                ...
                "selection_rule": "lowest median composite score prioritizing Mid-level reference-day stability, lower CV/drift, lower %bias dispersion, and preservation of Low/Mid/High separation",
            }])
